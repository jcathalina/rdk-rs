...
mod tests {
    ...
    fn test_mol_to_inchi() {
        ...
        let romol = ro_mol_ffi::smiles_to_mol(&smile).unwrap();
        let inchi = inchi_ffi::mol_to_inchi(romol);
    ...
    fn test_good_inchi_to_mol() {
        ...
        let romol = inchi_ffi::inchi_to_mol(&inchi, true, true).unwrap();
        assert!(!romol.is_null());
        assert_eq!(inchi_ffi::mol_to_inchi(romol.clone()), "InChI=1S/C2H6/c1-2/h1-2H3");
        assert_eq!(ro_mol_ffi::mol_to_smiles(romol), "CC");
    }

    #[test]
    fn test_bad_inchi_to_mol() {
        cxx::let_cxx_string!(bad_inchi = "asd");
        let romol = inchi_ffi::inchi_to_mol(&bad_inchi, true, true);
        assert!(romol.is_ok());
    ...
    fn test_good_inchi_to_inchi_key() {
        ...
        let inchi_key = inchi_ffi::inchi_to_inchi_key(&inchi).unwrap();
    ...
    fn test_bad_inchi_to_inchi_key() {
        cxx::let_cxx_string!(bad_inchi = "asd");
        let inchi_key = inchi_ffi::inchi_to_inchi_key(&bad_inchi);
