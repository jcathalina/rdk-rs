...
fn test_ro_mol() {
    ...
    let romol = rdk_sys::ro_mol_ffi::smiles_to_mol(&smile).unwrap();
...
fn bad_mol_test() {
    ...
    let romol = rdk_sys::ro_mol_ffi::smiles_to_mol(&smile);
...
fn parse_without_sanitize_test() {
    ...
    let params = rdk_sys::ro_mol_ffi::new_smiles_parser_params();
    rdk_sys::ro_mol_ffi::smiles_parser_params_set_sanitize(params.clone(), true);
    let romol = rdk_sys::ro_mol_ffi::smiles_to_mol_with_params(&smile, params);

    assert!(romol.is_err());

    let params = rdk_sys::ro_mol_ffi::new_smiles_parser_params();
    rdk_sys::ro_mol_ffi::smiles_parser_params_set_sanitize(params.clone(), false);
    let romol = rdk_sys::ro_mol_ffi::smiles_to_mol_with_params(&smile, params);

    assert!(romol.is_ok());

    let romol = romol.unwrap();
    let problems = rdk_sys::ro_mol_ffi::detect_chemistry_problems(romol);
    assert_eq!(
